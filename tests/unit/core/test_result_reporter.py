...
from pdf_merger.core.result_reporter import format_result_detailed, format_result_summary
...
class TestFormatResultSummary:
    ...
    def test_format_result_summary_success(self):
        ...
        result = MergeResult(total_rows=10, successful_merges=10, failed_rows=[])
    ...
    def test_format_result_summary_empty(self):
        ...
        result = MergeResult(total_rows=0, successful_merges=0, failed_rows=[])
...
class TestFormatResultDetailed:
    ...
    def test_format_result_detailed_with_mergeresult(self):
        ...
        result = MergeResult(total_rows=10, successful_merges=0, failed_rows=[], skipped_rows=[])
        ...
        result.add_row_result(
            RowResult(
                2, RowStatus.FAILED, error_message="File not found", files_missing=["GRNW_123"]
            )
        )
    ...
    def test_format_result_detailed_with_mergeresult_partial(self):
        ...
        result = MergeResult(total_rows=5, successful_merges=3)

        # Add partial result
        partial_result = RowResult(
            0, RowStatus.PARTIAL, files_found=[Path("/tmp/file1.pdf")], files_missing=["GRNW_123"]
        )
    ...
    def test_format_result_detailed_with_processing_time(self):
        ...
        result = MergeResult(total_rows=10, successful_merges=10, total_processing_time=5.5)
